...
amidebian8=`cat /etc/debian_version|grep ^8.|wc -l`
internalbridgepresent=`ovs-vsctl show|grep -i -c bridge.\*$integration_bridge`
kernel64installed=`uname -a|grep x86_64|head -n1|wc -l`
...
then
	echo ""
	echo "DEBIAN 8 O/S Verified OK"
	echo ""
else
	echo ""
	echo "We could not verify an DEBIAN 8 O/S here. Aborting !"
	echo ""
	exit 0
fi
...
DEBIAN_FRONTEND=noninteractive apt-get -y install pm-utils saidar sysstat iotop ethtool iputils-arping libsysfs2 btrfs-tools \
...
then
	...
	DEBIAN_FRONTEND=noninteractive apt-get -y install qemu kvm qemu-kvm libvirt-bin libvirt-doc
	...
	DEBIAN_FRONTEND=noninteractive apt-get -y install dnsmasq dnsmasq-utils
...
#apt-get -y install ksmtuned
dpkg -i libs/ksmtuned_4.20150325_amd64.deb

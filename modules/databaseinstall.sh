...
then
	...
	"postgres")
		...
		echo -e "host\tall\tall\t0.0.0.0 0.0.0.0\tmd5" >> /etc/postgresql/9.4/main/pg_hba.conf
		sed -r -i 's/^max_connections.*/max_connections\ =\ 1000/' /etc/postgresql/9.4/main/postgresql.conf
		/etc/init.d/postgresql restart
		systemctl restart postgresql 
		sleep 5
		sync
